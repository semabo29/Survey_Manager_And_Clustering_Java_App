...
public class DriverGeneral {
                ...
                if (leyendoRespuestas && linea.startsWith("RESPUESTA,")) {
                    ...
                    for (int i = 1; i <= numPreguntas; i++) {
                        ...
                        if (p.getTipoPregunta() instanceof FormatoLibre) {
                            ...
                            cd.addDatoRespuesta(i, new TDatosInteger(Integer.parseInt(val)));
                        }
    ...
    private static void ejecutarAnalisis() {
        try {
            ArrayList<ArrayList<RespuestaEncuesta>> clusters = cd.analizarEncuesta();

            float calidad = cd.getEvaluacionCalidad(clusters);
            System.out.println("---- RESULTADOS DEL ANÁLISIS ----");
            System.out.println("Número de clusters obtenidos: " + clusters.size());
            //por cada cluster se vera cada email de las respuestas que lo componen
            ...
            System.out.println("Calidad del clustering: " + calidad);
        } catch (Exception e) {
